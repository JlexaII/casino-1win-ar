...
(function () {
    "use strict";
    $(".linking_bl button").on("click", function () {
        let button = $(this);
        let menu_container = $(".linking_bl .linking_bl_menu");
        if (menu_container.hasClass("show")) {
...
(function () {
    "use strict";
    $("head").append('<link rel="stylesheet" href="' + app_js_localize.my_theme_url + '/assets/fontawesome/css/all.min.css">');
    let links = [];
    let anchors = $('a[href^="#"]');
    let inx_last_links = 0;
    ...
    anchors.on("click", function (e) {
        ...
        if ($(window).width() < 992) {
            if (inx) {
                if (inx < inx_last_links) {
                    offset += 118;
                } else {
                    let toc_navigation = $(".toc_navigation");
                    if (toc_navigation.length) {
                        offset += toc_navigation.height();
                    }
                }
                inx_last_links = inx;
            }
        ...
        $("html, body").animate({ scrollTop: $(anc).offset().top - offset });
    });
    let bottom_panel = $(".bottom_panel");
    $(window).on("scroll", function () {
        let scrollPos = $(document).scrollTop();
        let scrollToTopIcon = $("#wpfront-scroll-top-container");
        if (scrollPos > 600) {
            bottom_panel.addClass("show");
            if (bottom_panel.length > 0) {
                scrollToTopIcon.addClass("panel_mode");
            }
        } else {
            bottom_panel.removeClass("show");
            if (bottom_panel.length > 0) {
                scrollToTopIcon.removeClass("panel_mode");
            }
        }
    });
    $(".primary_menu_desktop_ver ul").dropdownMouse();
    let menu_arrow = $("header nav > ul > li .arrow");
    ...
    $("footer .nav_column .heading").on("click", function () {
        if ($(window).width() > 991) {
            return false;
        }
        let heading = $(this);
        let nav = $(this).next("nav");
    ...
    $(".mobile_menu_button").on("click", function () {
        let button = $(this);
        let menu_container = $("header.header .mobile_menu_container");
    ...
    $(".lang button").on("click", function () {
        let button = $(this);
        let menu_container = $(".lang .lang-menu");
        if (menu_container.hasClass("show")) {
            button.removeClass("active");
            menu_container.removeClass("show");
        } else {
    ...
    $(".faq-section .faq_question").on("click", function () {
        let item = $(this).parent(".item");
        item.toggleClass("show");
    });
    if ($(".image_slider_block").length > 0) {
        let splide_direction = app_js_localize.is_rtl ? "rtl" : "ltr";
        new Splide(".image_slider_block", { perPage: 3, direction: splide_direction, rewind: true, breakpoints: { 700: { perPage: 2 }, 500: { perPage: 1 } } }).mount();
    }
    if ($(".reviews_block").length > 0) {
        let splide_direction = app_js_localize.is_rtl ? "rtl" : "ltr";
        new Splide(".reviews_block", { perPage: 3, direction: splide_direction, rewind: true, breakpoints: { 991: { perPage: 1 } } }).mount();
    }
    $(".fn_link_href").on("click", function () {
        let url = $(this).attr("data-href");
        if (url) {
            window.location.href = decodeURIComponent(escape(window.atob(url)));
        }
    });
    $(".howto_block .how-list button, .howto_app .how-list button").on("click", function () {
        let id = $(this).attr("data-href");
        let cont = $(this).parents(".howto__container");
        $(".how-list button", cont).removeClass("active");
        $(".how-description .item", cont).removeClass("show");
        $(this).addClass("active");
        $(id, cont).addClass("show");
    });
    $(".howto_app .howto_tabs button").on("click", function () {
        let cont = $(this).parents(".howto_app");
        let howto_tabs = $(this).parents(".howto_tabs");
        let id = $(this).attr("data-href");
        $("button", howto_tabs).removeClass("active");
        $(this).addClass("active");
        $(".howto_tab", cont).removeClass("active");
        $(id, cont).addClass("active");
    });
    $(".bonus-items .item").on("click", function () {
        let container = $(this).parents(".bonus-items");
        let item = $(this);
        $(".item", container).removeClass("active");
        item.addClass("active");
    });
    $(".spoiler-block button").on("click", function () {
        let item = $(this).parents(".spoiler-block");
        let name1 = $(this).attr("data-name1");
        let name2 = $(this).attr("data-name2");
        let button = $("span", this);
    ...
    $(".lwptoc .lwptoc_toggle_arrow").on("click", function () {
        let container = $(this).parents(".lwptoc");
        let header = $(".lwptoc_header", container);
        let lwptoc_items = $(".lwptoc_items", container);
        if (lwptoc_items.is(":hidden")) {
            header.addClass("show");
            lwptoc_items.css("display", "block");
        } else {
            header.removeClass("show");
            lwptoc_items.css("display", "none");
        }
    });
...
$(document).on("click", ".promocode-page .welcome-section .bonus_powitalny-cupon_bonus", function () {
    const promoCode = $(this);
    const span = promoCode.find("span");
    const icon = promoCode.find("i");
    const textToCopy = span.text();
    ...
    const popup = $(".promocode-page .welcome-section .bonus_powitalny-cupon_bonus-popup");
    popup.addClass("popup_active");
    const popupClose = popup.find("i");
...
(function () {
    "use strict";
    let slots_nav = $(".slots .nav");
    $(".slots .nav li:first-child button").addClass("active");
    $(".slots .items .slot:first-child").addClass("show");
    $(".slots .nav li button").on("click", function () {
        let cont = $(this).parents(".slots");
        let nav = $(this).parents(".nav");
        let id = $(this).attr("data-href");
        if (nav.hasClass("mousemove")) {
            nav.removeClass("mousemove");
            return false;
        }
        $(".nav li button", cont).removeClass("active");
        $(".items .slot", cont).removeClass("show");
        $(this).addClass("active");
        $(id, cont).addClass("show");
    });
    $(".slots .items li").on("click", function () {
        let cont = $(this).parent("ul");
        let url = cont.attr("data-href");
        if (url) {
            window.location.href = decodeURIComponent(escape(window.atob(url)));
        }
    });
    slots_nav.on("mousedown", function () {
        const startX = this.scrollLeft + event.pageX;
        const startY = this.scrollTop + event.pageY;
        slots_nav.on("mousemove", function () {
            slots_nav.addClass("mousemove");
            this.scrollLeft = startX - event.pageX;
            this.scrollTop = startY - event.pageY;
            return false;
        });
    });
    slots_nav.on("mouseleave", function () {
        slots_nav.off("mousemove");
        slots_nav.removeClass("mousemove");
    });
    slots_nav.on("mouseup", function () {
        slots_nav.off("mousemove");
    });
    slots_nav.on("click", function () {
        $(this).removeClass("mousemove");
    });
})(jQuery);
function hb_popups_setCookie(name, value, days) {
    var expires = "";
    if (days) {
        var date = new Date();
...
(function () {
    "use strict";
    let hb_popups_bonus = $(".hb_popups_bonus");
    if (!hb_popups_bonus_localize.delay) {
        return false;
    }
    setTimeout(function () {
        hb_popups_bonus.css("display", "flex");
    }, hb_popups_bonus_localize.delay * 1000);
    $(".close", hb_popups_bonus).on("click", function () {
        hb_popups_bonus.css("display", "none");
        hb_popups_setCookie("hb_popups", 1, hb_popups_localize.cookie);
    });
    $("button", hb_popups_bonus).on("click", function () {
    ...
    function t(t) {
        var n = o("#wpfront-scroll-top-container"),
            ...
            a = function () {
                clearTimeout(i),
                    n.is(":visible") &&
                    n.stop().fadeTo(t.button_fade_duration, 0, function () {
                        n.hide(), (e = !1);
                    });
            },
            c = function () {
            ...
            r = function () {
                l ||
                    ((l = !0),
                        o(window).scrollTop() > t.scroll_offset
                            ? (n
                                .stop()
                                .css("opacity", e ? 1 : t.button_opacity)
                                .show(),
                                e || c())
                            : a(),
                        (l = !1));
            ...
            n
                ...
                .on("click", function (n) {
                    ...
                    if ("element" === t.button_action) {
                        ...
                        return i.animate({ scrollTop: l }, t.scroll_duration), !1;
                    }
                    return n.preventDefault(), o("html, body").animate({ scrollTop: 0 }, t.scroll_duration), !1;
                });
    }
    var o = jQuery;
...
$(".bonus_items2 .single_url_mode").on("click", function () {
    let item = $(this).parents(".bonus_items__container");
    let url = item.attr("data-url");
    if (url) {
        window.location.href = decodeURIComponent(escape(window.atob(url)));
    }
});
$(".bonus_items2 .show_more button").on("click", function () {
    let item = $(this).parents(".bonus_items2");
    let name1 = $(this).attr("data-name1");
    let name2 = $(this).attr("data-name2");
    let button_name = $("span", this);
    item.toggleClass("show_more");
    if (item.hasClass("show_more")) {
        button_name.html(name2);
    } else {
        button_name.html(name1);
        $("html, body").animate({ scrollTop: item.offset().top - 140 }, 0);
    }
});
$(".game_section .game_section__container .single_url_mode").on("click", function () {
    let item = $(this).parents(".game_section__container");
    let url = item.attr("data-url");
    if (url) {
        window.location.href = decodeURIComponent(escape(window.atob(url)));
    }
...
!(function () {
    var o,
        ...
        c =
            ((o = { duration: 300, action: "close", startTime: null, startHeight: null, endHeight: null, easing: t }),
                (l = function (e, i) {
                ...
                (n = function (t, e, i) {
                    ...
                    var n,
                        o = i - e.startTime;
                    o < e.duration
                        ? ((t.style.height = ((e.endHeight - e.startingHeight) * e.easing(o / e.duration) + e.startingHeight).toFixed(2) + "px"), l(t, e))
                        : ("close" === e.action && (t.style.display = "none"), "open" === e.action && (t.style.display = "block"), ((n = t).style.height = null), (n.style.overflow = null));
                }),
                function (t, e) {
                    if (window.requestAnimationFrame) {
                        var i = m(o, {});
                        (i.action = e),
                            t.style.height ? (i.startingHeight = parseFloat(t.style.height)) : (i.startingHeight = "close" === e ? t.scrollHeight : 0),
                            ((n = t).style.display = "block"),
                            (n.style.overflow = "hidden"),
                            (i.endHeight = "close" === e ? 0 : ((t.style.height = "0px"), t.scrollHeight)),
                            l(t, i);
                    } else t.style.display = "close" === e ? "none" : "block";
                    var n;
                }),
        a = function (t) {
            for (var e, i = document.querySelectorAll('[id="' + t + '"]'), n = 0; n < i.length; n++) if ((e = i[n]).offsetWidth || e.offsetHeight || e.getClientRects().length) return i[n];
            return null;
        },
        i =
                            ...
                            (e !== document.location.hash &&
                                (i.onComplete = function (t, e) {
                                    ...
                                    d.setAttribute("id", ""),
                                        ...
                                        s.setAttribute("style", "position:absolute;visibility:hidden;left:" + n + "px;top:" + o + "px;"),
                                        (l = document.body),
                                        (a = s),
                        ...
                        r.addEventListener("click", function (t) {
                            ...
                            t.preventDefault(),
                                ...
                                -1 < (" " + s.className + " ").indexOf(" " + l + " ")
                                    ? ((o = "lwptoc_items-visible"), ((n = s).className = (" " + n.className + " ").replace(" " + o + " ", "").trim()), c(s, "close"))
                                    : ((i = "lwptoc_items-visible"), ((e = s).className = e.className.trim() + " " + i), c(s, "open"));
                        });
                    "1" === t.getAttribute("data-smooth-scroll") && u.registerScrollTrigger(s.getElementsByTagName("A"), { offset: t.getAttribute("data-smooth-scroll-offset") });
                }
            },
            globalInit: function () {
